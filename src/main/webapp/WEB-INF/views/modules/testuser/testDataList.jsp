...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/testuser/testData/">测试列表</a></li>
		<shiro:hasPermission name="testuser:testData:edit"><li><a href="${ctx}/testuser/testData/form">测试添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="testData" action="${ctx}/testuser/testData/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>更新时间</th>
				<th>备注信息</th>
				<shiro:hasPermission name="testuser:testData:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="testData">
			<tr>
				<td><a href="${ctx}/testuser/testData/form?id=${testData.id}">
					${testData.user.name}
				</a></td>
				<td>
				...
				<td>
					<fmt:formatDate value="${testData.updateDate}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
					${testData.remarks}
				</td>
				<shiro:hasPermission name="testuser:testData:edit"><td>
    				<a href="${ctx}/testuser/testData/form?id=${testData.id}">修改</a>
					<a href="${ctx}/testuser/testData/delete?id=${testData.id}" onclick="return confirmx('确认要删除该测试吗？', this.href)">删除</a>
