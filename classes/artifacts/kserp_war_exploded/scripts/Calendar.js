/*
	支持的日期格式: yyyy-MM-dd; yyyy/MM/dd; yyyyMMdd
 	日期控件使用：
 	<INPUT type=text name=txt onfocus=calendar()> 使用默认格式
 	<INPUT type=text name=txt onfocus=calendar('yyyymmdd')> 使用(20060901)格式:yyyyMMdd
 	
 	使用默认格式:
 	<INPUT type=text name=txt onfocus=calendar() name=txt>
 	<INPUT type=button value='日期' onclick="calendar(document.form1.txt, null)"> 
 	
 	使用指定日期格式:
 	<INPUT type=text onfocus=calendar('yyyymmdd') name=txt1>
 	<INPUT onclick="calendar(document.form1.txt1, 'yyyy/mm/dd')" type=button value='日期'>
*/
document.write("<div id=commonCalendarLayer style='position: absolute; z-index: 9999; width: 144; height: 193; display: none'>");
document.write("<iframe name=commonCalendarIframe scrolling=no frameborder=0 width=100% height=100%></iframe></div>");
function writeIframe()
{
    ...
    "td#commonYearHead, td#commonYearMonth{color: "+ WebCalendar.wordColor +"}"+
    ...
    "</style></head><body onselectstart='return false' style='margin: 0px' oncontextmenu='return false'><form name=dateInput>";
    ...
    "    <td width=60 id=commonYearHead  title='点击此处选择年份' onclick='parent.funYearSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
    "    <td width=50 id=commonYearMonth title='点击此处选择月份' onclick='parent.funMonthSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=1 cellpadding=0>";
         ...
         for(var y=0; y<7; y++)  strIframe += "<td class=out id='commoDay"+ (x*7+y) +"'></td>"; strIframe += "</tr>";}
         strIframe += "<tr>";
         for(var x=35; x<39; x++) strIframe += "<td class=out id='commoDay"+ x +"'></td>";
         ...
         " type=button value='&nbsp;清空 &nbsp;' onclick='parent.clearCalendar()'> "+
    ...
    "    onfocus='this.blur()' style='dateInput:expression(this.disabled=parent.WebCalendar.thisYear==1000)'><input"+
    ...
    "    onfocus='this.blur()' style='dateInput:expression(this.disabled=parent.WebCalendar.thisYear==9999)'></td></tr></table>"+
    ...
    with(WebCalendar.iframe)
    {
        ...
        for(var i=0; i<39; i++)
        {
            WebCalendar.dayObj[i] = eval("commoDay"+ i);
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.objExportYear = null; 					//日历回传的年显示控件
    this.objExportMonth = null;					//日历回传的月显示控件
    this.objExportDay = null;					//日历回传的日显示控件
    ...
    this.iframe     = window.frames("commonCalendarIframe"); //日历的 iframe 载体
    this.calendar   = getObjectById("commonCalendarLayer");  //日历的层
    this.dateReg    = "";           //日历格式验证的正则式
    this.yearFall   = 10;           //定义年下拉框的年差值
    this.format     = "yyyy-mm-dd"; //回传日期的格式 yyyy-mm-dd;yyyy/mm/dd; yyyymmdd
    this.timeShow   = false;        //是否返回时间
    this.drag       = true;         //是否允许拖动
    this.darkColor  = "#d00000";    //控件的暗色
    this.lightColor = "#FFFFFF";    //控件的亮色
    this.btnBgColor = "#fae6e6";    //控件的按钮背景色
    this.wordColor  = "#000080";    //控件的文字颜色
    this.wordDark   = "#DCDCDC";    //控件的暗文字颜色
    this.dayBgColor = "#F5F5FA";    //日期数字背景色
    this.todayColor = "#FF00FF";    //今天在日历上的标示背景色
...
{	
 ...
 if (arguments.length == 0) WebCalendar.objExport = e; //没有参数，直接以默认格式处理
 else if (arguments.length == 1) {
 	WebCalendar.format=arguments[0]; //一个参数时以默认方式处理
 	WebCalendar.objExport = e;
 } else if (arguments.length == 4) {
 	WebCalendar.objExport = eval(arguments[0]);
 	WebCalendar.objExportYear = eval(arguments[1]);
 	WebCalendar.objExportMonth = eval(arguments[2]);
 	WebCalendar.objExportDay = eval(arguments[3]); 	
 } else { //输入框；格式  
   	WebCalendar.objExport = eval(arguments[0]); 	
...
function hiddenCalendar(){getObjectById("commonCalendarLayer").style.display = "none";};
function clearCalendar(){WebCalendar.objExport.value=""; hiddenCalendar();
};
...
function writeCalendar() //对日历显示的数据的处理程序
{
    ...
    WebCalendar.iframe.commonYearHead.innerText  = y +" 年";
    WebCalendar.iframe.commonYearMonth.innerText = parseInt(m, 10) +" 月";
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        if (returnValue.length == 10) {
           if (WebCalendar.objExportYear != null) {
	        WebCalendar.objExportYear.value = returnValue.substring(0,4);
    	    	WebCalendar.objExportMonth.value = returnValue.substring(5,7);
        	WebCalendar.objExportDay.value = returnValue.substring(8,10);
          }
        }
        hiddenCalendar();
    }
