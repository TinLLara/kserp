	...
	<script type="text/javascript">
		...
		function doRefuse(){
			...
			if(checkRemark()){
				...
				if(nextnoder!='${oaBusitrip.applyperson}'){
					alert("温馨提示：请选择申请者作为下一处理人，然后拒绝。");
					return;
				}
				var id = '${oaBusitrip.busitripid}';
				//var url = '${ctx}'+"/holi/oaHolidayapply/check?tag=refuse&remark="+encodeURIComponent(remark)+'&id='+happlyid;
				var url = '${ctx}'+"/oabustrip/oaBusitrip/check?tag=refuse&busitripid="+id+"&remark="+remark;
				url = encodeURI(url);
				url = encodeURI(url);
				window.location = url;
			}
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/oabustrip/oaBusitrip/listforcheck">出差申请审批列表</a></li>
		<li class="active"><a href="${ctx}/oabustrip/oaBusitrip/formforcheck?busitripid=${oaBusitrip.busitripid}">出差申请<shiro:hasPermission name="oabustrip:oaBusitrip:edit">${not empty oaBusitrip.busitripid?'审批':'审批'}</shiro:hasPermission><shiro:lacksPermission name="oabustrip:oaBusitrip:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="oaBusitrip" action="${ctx}/oabustrip/oaBusitrip/check?tag=pass" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<%-- <div class="control-group">
			...
			<div class="controls">
				...
				<input type="text" id="nextnoder" value="" name="nextnoder"  readonly="readonly" style="width:120px;background:#F9F9F9;border:0px;color:red;" />
		...
		<c:choose>
			<c:when test="${tagvalue eq 'view'}">
				<!-- <div class="control-group">
					...
					<div class="controls">
						<textarea id="remark" name="remark" class="required" disabled="disabled"></textarea>
						<span class="help-inline"><font color="red">*</font> </span>
					</div>
				</div> -->
			...
			<c:otherwise>
				<!-- <div class="control-group">
					<label class="control-label">审批意见：</label>
					<div class="controls">
						<textarea id="remark" name="remark" class="required" ></textarea>
						<span class="help-inline"><font color="red">*</font> </span>
					</div>
				</div> -->
				<div class="form-actions">
