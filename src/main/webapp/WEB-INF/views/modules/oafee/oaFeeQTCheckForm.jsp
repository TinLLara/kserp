...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/oafee/oaFee/formforcheck?feecat=${oaFee.feecat}">费用管理列表</a></li>
		<li class="active"><a href="${ctx}/oafee/oaFee/formforcheck?feecat=QT&feeid=${oaFee.feeid}">费用管理<shiro:hasPermission name="oafee:oaFee:edit">${not empty oaFee.feeid?'审批':'审批'}</shiro:hasPermission><shiro:lacksPermission name="oafee:oaFee:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
				...
				<c:choose>
					<c:when test="${not empty oaFee.feeid}">
						<c:forEach items="${otherfeeList}" var="OaOtherfee">
							<tr>
							<td class='content_body' align="center" width="15%"><input type="hidden" value="${OaOtherfee.atmtype}" name="proid[]" id="id" /><select name="feetypes[]" id="feetypes[]" style="width:145px;" onchange="changeType(this.value,'')">
						...
						<c:choose>
							...
							<td class='content_body' align="center" width="15%"><input type="text" name="ohappentime[]" id="ohappentime[]" value="${OaOtherfee.haptime}" class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="width:185px;" /></td>
							<td class='content_body' align="center" width="12%"><input type="text" name="oldatms[]" id="oldatms[]" value="${OaOtherfee.oldcash}" onkeyup="countTotal();" style="width:80px;" size="20" /></td>
							<td class='content_body' align="center"><input type="text" name="reason[]" id="reason[]" value="${OaOtherfee.reason}" style="width:96%;" /></td>
							</tr>
						</c:forEach>
					</c:when>
					<c:otherwise>
